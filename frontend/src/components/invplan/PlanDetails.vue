<template>
  <div class="flex flex-col px-4 pt-4 pb-4">
    <div class="flex">
      <div class="w-1/2">
        <DayPlanComponent :percentage_change="plan_percentage_change" :labels="plan_labels" :values="plan_percentages"
          :planName="plan_name" />
      </div>
      <div class="w-1/2 flex flex-col gap-6">
        ...
        <div v-if="auth.isLoggedIn" class="flex justify-end">
          ...
          <Button v-if="Number(auth.user_data.plan_id) !== planId" label="Elegir Plan" @click="selectPlan" />
          <!-- Si ya es ese plan, mostramos mensaje deshabilitado -->
          <Button v-else label="Plan seleccionado" disabled />
        </div>
      </div>
    </div>
  </div>
